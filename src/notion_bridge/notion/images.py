"""Native Notion image attachments for OpenAI Responses image inputs."""
...
def decode_response_image(image_url: Any, index: int) -> ResponseImage:
    if not isinstance(image_url, str) or not image_url.startswith("data:"):
        raise ImageInputError(
            "input_image.image_url must be a base64 data URL; "
            "Codex CLI -i/--image uses this format"
        )
    try:
    ...
    for item in request_input:
        ...
        for part in content:
            ...
            if len(images) >= MAX_IMAGE_COUNT:
                raise ImageInputError(
                    f"at most {MAX_IMAGE_COUNT} images are supported per request"
                )
