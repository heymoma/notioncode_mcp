...
from notion_bridge.accounts.pool import (
...
class AccountPoolTests(unittest.IsolatedAsyncioTestCase):
    async def test_times_out_runaway_inference_and_releases_account(self) -> None:
        pool = NotionAccountPool(
            [FakeClient("one")],
            account_ids=["one"],
            limits=PoolLimits(inference_timeout_seconds=0.01),
        )
...
class AccountDiscoveryTests(unittest.TestCase):
    ...
    def test_ignores_surplus_accounts_instead_of_refusing_to_start(self) -> None:
        # Raising here used to crash-loop the whole service because one extra
        # account file was present, which is a purely additive mistake.
        with tempfile.TemporaryDirectory() as temporary_directory:
            home = Path(temporary_directory)
            ...
            self.assertEqual(pool.skipped_accounts, 3)
            self.assertEqual(pool.discovered_accounts, 13)

    def test_max_accounts_can_be_lowered_for_a_constrained_deployment(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            home = Path(temporary_directory)
            ...
            pool = build_account_pool(home, max_accounts=2)

            self.assertEqual(pool.size, 2)
            self.assertEqual(pool.skipped_accounts, 2)
